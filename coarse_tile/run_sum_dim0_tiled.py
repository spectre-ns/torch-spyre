...
"""Standalone: x.sum(dim=0) tiled over B.

Mirrors test_hint_tiled_reduction_dim0_sum_correct in test_coarse_tile_e2e.py.

Reduces a [B, D] tensor over B (dim=0), producing a [D] output where D
sits on the stick.  spyre_hint(num_tiles_per_dim={"B": 4}).

Usage:
    python3 tests/inductor/standalone/run_sum_dim0_tiled.py
"""

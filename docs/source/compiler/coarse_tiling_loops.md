...
- `buf1` (`z`'s tile-sized producer) is planned `kind="copy_out"` rather than
  ...
  which is what lets `plan_core_division_and_scratchpad` place it in `lx`
  rather than falling back to `hbm_pool` — see the OpSpec and `bundle.mlir`
  sections below.
...
- **All five tile-sized buffers fit in `lx`; none fall back to `hbm_pool`.**
  ...
  iteration. `plan_core_division_and_scratchpad`'s allocator fits all of
  them in LX scratchpad simultaneously at this tile size (`[512, 1024]`
  fp16, 4 cores), so no buffer here ever needs the bulk `hbm_pool` fallback
  (`constants.py`'s `INTERMEDIATES_SEGMENT`, assigned by the later
  `hbm_pool_planning` pass). Whether a given buffer lands in `lx` or spills
  to `hbm_pool` is a placement decision made per compilation by
  `plan_core_division_and_scratchpad`, based on how much same-lifetime
...
- The final output `z` (output of `mul`) has no inside consumers, so it
  ...
  the copy, and `plan_core_division_and_scratchpad` places it in `lx`
  (address 0, aliasing `y`'s slot since `y` and `mul`'s output are never
  live at the same time within scratchpad's allocator).  The identity copy is still the op whose
...
- HBM inputs `a`, `b`, `c` also have `device_size=[64, 1024, 64]` — the full
  ...
  `plan_core_division_and_scratchpad` would leave whichever ones spill
  without an `lx` slot, and `hbm_pool_planning` would later give them
  `allocation={'hbm_pool': ...}` in the bulk-allocated HBM region
...
2. **Allocate `accum_tile`** (per-tile scratch, same per-tile output shape)
   ...
   address; `plan_core_division_and_scratchpad` can therefore place it in LX
   scratchpad memory.

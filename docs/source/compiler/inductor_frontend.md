...
| [`scratchpad/`](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/scratchpad/) | `run_optimization` (the pipeline entry point, formerly `scratchpad_planning`), the core-division pre-passes (`SpanReductionPass`, `WorkDistributionPass`), and the layout-solver framework (`GreedyLayoutSolver`, `FirstFitLayoutSolver`, `BestFitLayoutSolver`). Core division and LX scratchpad allocation. |

...
| 16 | `plan_core_division_and_scratchpad` | [scratchpad/allocator.py](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/scratchpad/allocator.py) | Core division and LX allocation in one stage. `SpanReductionPass` and `WorkDistributionPass` (wrapping [work_division.py](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/work_division.py)) run as the allocator's pre-optimization passes on **every** compile, then `config.lx_planning` gates only the scratchpad placement that follows. |
...
| [`work_division.py`](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/work_division.py) | `span_reduction`, `cost_model_matmul_division`, `work_distribution`, `divide_pointwise_op`, `divide_reduction_op`, `apply_splits`. Three-pass work division: span reduction (mandatory), cost-model matmul division (claims a subset of matmuls), and work distribution (covers the rest). Driven by the scratchpad allocator's pre-optimization passes, not by the pipeline directly. |
| [`scratchpad/`](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/scratchpad/) | `plan_core_division_and_scratchpad` (the pipeline entry point, formerly `scratchpad_planning`), the core-division pre-passes (`SpanReductionPass`, `WorkDistributionPass`), and the layout-solver framework (`GreedyLayoutSolver`, `FirstFitLayoutSolver`, `BestFitLayoutSolver`). Core division and LX scratchpad allocation. |

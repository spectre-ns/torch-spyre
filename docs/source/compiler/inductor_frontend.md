...
| 16 | `_core_division_and_lx_planning` | [scratchpad/](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/scratchpad/) | Core division and LX allocation in one stage. `SpanReductionPass` and `WorkDistributionPass` (from [work_division.py](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/work_division.py)) run as the allocator's pre-optimization passes, then `config.lx_planning` gates the scratchpad placement that follows. |
...
| [`scratchpad/`](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/scratchpad/) | `scratchpad_planning`, the core-division pre-passes (`SpanReductionPass`, `WorkDistributionPass`), and the layout-solver framework (`GreedyLayoutSolver`, `FirstFitLayoutSolver`, `BestFitLayoutSolver`). Core division and LX scratchpad allocation. |

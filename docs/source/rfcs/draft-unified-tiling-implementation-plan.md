...
## 0. Status — what has landed, what has not

Verified against the object store on 2026-09-04, not from memory: blob hashes are
quoted so a reader can re-check a claim with `git cat-file`.

### 0.1 Landed upstream

| Artifact | Blob | Size | On |
|---|---|---|---|
| `scratchpad/coarse_tiling.py` (`CoarseTilingPass`, `tile_spec_to_dim_hints`, `derive_tiling_groups`) | `ae760bda` | 9718 B | `upstream/main`, `main`, working branch — identical |
| `wsr/enumerate_tilings.py` (`enumerate_tile_options`) | `7d425665` | 11015 B | same, and byte-identical to `coarse-tiling-stage3` |
| `TileSpec` / `TileAxis`, `CoreDivision.tiling`, `min_footprint` tile factor (`plan_solver.py`) | — | — | working branch |
| `ScratchpadOptimizationPass` hook (`allocator.py:202`) | — | — | working branch |

**Stages 1 and 3 are therefore done.** Their code is in the mainline and their
gate suites pass headlessly on the working branch (no card required):
`test_coarse_tiling.py` 353 passed / 1 skipped, `test_enumerate_tilings.py` 13
passed, both run 2026-09-04.

**But both are unwired, and that is the important qualification.** Nothing
constructs `CoarseTilingPass`; `enumerate_tile_options` has test callers only;
neither `unified_tiling` nor `auto_coarse_tiling` exists in `config.py` on the
mainline. The landed machinery is inert — a tiling still reaches the graph only
through the two pre-stickification hint passes (`_maybe_coarse_tile_hints`,
`_maybe_coarse_tile_span_overflow`). So stage 1's capability exists and is
unreachable, which is exactly the "adds unwired machinery" half of the staging
contract, and it stays that way until stage 5 lands the caller.

One consequence to watch: `_is_coarse_tiled` (`allocator.py:1266`) is reachable
only through `_drop_reduction_splits_in_coarse_group`. The division *pin* on
coarse-tiled ops — the one that fixes the fused-group division inconsistency — is
**not** in `_division_map`'s guard ladder on this branch. It exists on another
branch and has not come across.

### 0.2 Built but unlanded
...
A trap for anyone reading both. The branch `coarse-tiling-stage6` is **not** this
plan's "Stage 6 — Padding". It carries hints-as-`TileSpec`s
(`dim_hints_to_tile_spec`, carried pins, WSR standing down when the solver owns
tiling), which is the enablement half of **plan stage 5**. Padding has not been
started. Read branch names as work order, never as stage identity.

### 0.4 What the 2026-08-25 decision actually withdrew

The recorded decision is that coarse tiling is left **unquantified** in the
CP-SAT objective — every tiling carries equal weight and competes only through LX
residency and work division. Read that narrowly, because the tree does *not* say
the cost seam was removed.

- **The generic cost-expression seam is live in mainline.**
  `CoOptimizingAllocator` builds a symbolic cost with `predict_by_bundle`,
  sympifies it, and passes it to `plan_layout_and_core_divisions`
  (`allocator.py:2007-2017`). `_minimize_cost_expr`
  (`ilp_solver_ortools.py:764`) minimizes it when it linearizes and falls back
  to the lexicographic ladder when it does not (`:826`). **Plan stage 2's
  deliverable — "put the injection point in place" — therefore exists in the
  tree**, by a different route than stage 2 described.
- **What was withdrawn is coarse tiling's own objective term.** No `tile_count`
  or `full_size` cost contribution is injected and tilings are not priced against
  each other. That is the decision, and it is what makes stage 5 simpler.
- **Stage 7 was not executed as written.** R3.2 specified *replacing* the
  two-phase lexicographic solve with one weighted `Minimize`. What exists is a
  weighted `Minimize` used only when a prediction is available, with the ladder
  retained as fallback. The mechanism exists; the replacement did not happen.

Tree corroboration for the decision, which is otherwise a recorded fact rather
than something visible in the code: `tests/inductor/test_cost_expr.py` is present
on `coarse-tiling-stage4/5/6` — the three branches whose base predates it — and
absent from every branch rebuilt after it (`stage1`, `stage2`, `stage3`, `cost`)
and from `upstream/main`.

> **Confirm this subsection before acting on it.** Withdrawing stages on a
> recorded decision is a large call, and the document rewrite that decision
...
| Stage | What lands | Status | Requirements | Gate |
|---|---|---|---|---|
...
## Stage 2 — The cost-function seam *(SUPERSEDED — see §0.4)*

> **Not a deliverable, because the seam already exists.** `CoOptimizingAllocator`
> already injects a `predict_by_bundle` cost expression
> (`allocator.py:2007-2017`, `ilp_solver_ortools.py:764`). What the 2026-08-25
> decision withdrew is narrower: coarse tiling gets no objective term of its own.
> The section is kept because its argument — that the retained two-phase solve
> already expresses tiling's benefit without being told to — is *why* that
> decision went the way it did, and stage 5 still rests on it.
...
| **No new wrapper.** Pairing the tiling onto the candidate (*Architecture*) means one index still selects the joint choice, so `_CoreDivisionBufferWithCpVars` (`:243`) needs only two more per-candidate derived scalars (`tile_count`, `full_size`), read by the retained objective through `eff_size`. **Under the amended M4 those are two more terms in the linear expression over the candidate one-hot, not two more `AddElement` tables** — see *Roadmap alignment*. The two-level `tile`/`div` index and its `AddAllowedAssignments` are **withdrawn** — there is no pair to constrain, and no symbol binding happens at all (stage 7 is out of scope, §0.4) | `ilp_solver_ortools.py` | R4.8 (half) |
...
## Stage 7 — The M2 objective collapse *(NOT AS SPECIFIED — see §0.4)*

> **Not a deliverable as written.** R3.2's *replacement* of the two-phase
> lexicographic solve never happened; what exists is a single weighted `Minimize`
> used when a prediction is available, with the ladder retained as fallback. Any
> later stage that says "symbol binding waits for stage 7" should be read as:
> that binding does not happen.
...
This plan is collateral document 1 of the compiler-optimization roadmap
(`draft-compiler-optimization.md`) and executes its **Phase 1**. Four points
where the two must be read together.

### The M4 encoding rule has been amended

M4 used to read "any `f(candidate) -> int` becomes a table entry plus one
`AddElement`." It now reads **"a table column plus one term in a linear
expression over the candidate one-hot"**: the operation carries
`exactly_one(z)` over its candidates and each derived scalar is
`sum_i f(candidate_i) * z[i]`.

The amendment is measured, not stylistic — 1.64x corpus-total on the three
capture corpora at `num_search_workers = 8`, with all three lexicographic
objective values and the resident set identical on all 15 graphs
(`draft-constraint-core-division.md`). The reason is that a channeled scalar is a
linear expression the objective's LP relaxation can see through, where an
`AddElement` result is opaque to it.

For this plan the change is mechanical and touches stage 5 only: `tile_count` and
`full_size` become two more coefficient columns over the same one-hot. It is
strictly easier than what was planned, and it removes the last reason stage 5
needed a solver-side table of its own. The RFC's own M4 citations
(`draft-unified-tiling-cpsat.md` §1, R2, and the "only extension mechanism"
paragraph) carry the superseded wording and should be read through this section.

### Phase 5's separability limit was too pessimistic, which helps stage 4

The roadmap's Phase 5 recorded that per-dimension channeling "pays only where
every derived scalar is separable per dimension", and that "anything needing the
full per-core view does not — and that is exactly what phase 3 consumes." That
has been revised: `PerCoreView` **does** separate, into a split factor `F_d` and
a core-mapping stride `sigma_d` per *buffer device dimension*, plus `cores_used`.
The supports are that `ResidencyEdge.compatible` is a key equality by
construction (rectangular on all 607 captured edges) and that
`core_to_slice_mapping` builds strides as prefix products.

Stage 4 is the stage that predicts tiled-frame views, so this is the stage that
benefits: a predicted view can be compared field-by-field against a recomputed
one in the same decomposition the solver uses, rather than as an opaque object.
It does not change stage 4's gate, which is already "predicted view equals the
view recomputed after `coarse_tile`".

### One genuine divergence: how loop-nest grouping is decided

Phase 1 specifies "one boolean per adjacent pair of operations deciding whether a
loop nest breaks there", with grouping falling out of the objective "rather than
being precomputed by a grouping heuristic that a wrong guess would make
unrecoverable."

The landed implementation does not do that. `derive_tiling_groups`
(`coarse_tiling.py:120`) groups **maximal consecutive runs of ops sharing the
same non-empty `TileSpec`**, and contiguity is a hard requirement enforced twice
(`validate_coarse_tile_groups` and `_apply_plan`).

The gap is narrower than it first looks, and worth stating precisely rather than
raising as an alarm. Because the solver chooses a `TileSpec` per operation, "a
maximal run sharing a spec" and "a maximal run with no break" coincide wherever
adjacent specs differ — the break boolean is *implied* by spec inequality, so
grouping is a function of a decision the solver does make. What is genuinely lost
is one degree of freedom: two adjacent operations that choose the *same* spec are
forced to fuse into one loop nest, and there is no way to express "same tiling,
separate nests". Whether that freedom is worth an `O(ops)` boolean is an open
question this plan does not answer; it should be decided on measurement, and
until then Phase 1's variable list overstates what is built.

### What Phase 1's other elements map to

| Phase 1 element | Where it lives here | Status |
|---|---|---|
| Baseline candidate (M3) — today's `_combo_cost`-ranked first feasible tiling | **Not landed.** `enumerate_tile_options` guarantees the *untiled* `TileSpec` is first and never truncated (`_finalize_options`), which is not the same candidate: today's behaviour on the span-overflow path can be a non-empty tiling, and that specific spec is not enumerated | open |
| Hint form (H1/H2) — `tiles`, `slices`, `num_tiles_per_dim`, `work_div` | `tile_spec_to_dim_hints` / `dim_hints_to_tile_spec` | half landed; the reverse direction is on `coarse-tiling-stage6` |
| Validator (H3) — hint survives enumeration, scope may not straddle a break | `validate_coarse_tile_groups` | landed, but only the contiguity half |
| Padding as a derived scalar | Stage 6 | not started, conditional on R10.3 |
| Sizing — tile determines predicted sizes and lifetimes | `min_footprint`'s tile factor + stage 4's predictor | half landed |

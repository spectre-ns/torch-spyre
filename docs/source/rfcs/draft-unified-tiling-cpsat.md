...
> form is:
>
> 1. **Coarse tiling gets no objective term.** By the 2026-08-25 decision it is
>    left *unquantified* — every tiling carries equal weight and competes only
>    through LX residency and work division. Note the narrowness: the *generic*
>    cost-expression seam is alive and wired in the tree
>    (`allocator.py:2007-2017`, `ilp_solver_ortools.py:764`), so plan stage 2's
>    deliverable exists by another route, and stage 7's specified *replacement*
>    of the lexicographic ladder never happened. R3.1–R3.7 should be read against
>    plan §0.4 rather than taken as written.
> 2. **M4's binding rule changed.** Where this document says a derived scalar is
>    bound "by `AddElement`" (§1, R2, the "only extension mechanism" paragraph,
>    and the `full_size` / `tile_count` items), the roadmap's M4 now reads *a
>    table column plus one term in a linear expression over the candidate

...
def _layout_dim_order(td: "TensorDep", shared: set[Symbol]) -> list[typing.Hashable]:
    """``td``'s device-dim order, projected onto ``shared`` symbols and its stick.

    The stick-count coordinate becomes a single ``"stick"`` token whichever
    variable indexes it (the contraction for an activation, the output axis for
    the result). Every other coordinate contributes the ``shared`` symbols it
    reads; coordinates reading none (constants, operand-only axes) are dropped.
    The trailing within-stick coordinate carries no order and is skipped.
    """
    stick_syms = td.device_coords[-1].free_symbols
    order: list[typing.Hashable] = []
    for coord in td.device_coords[:-1]:
        syms = coord.free_symbols
        if syms & stick_syms:
            order.append("stick")
        elif syms & shared:
            order.append(frozenset(syms & shared))
    return order


def matmul_k_split_layout_order_domains(
    ctx: WorkDivConstraintContext,
) -> ConstraintResult:
    """Restrict matmul K to split=1 when an operand's dim order differs from the output's.

    A K-split hands each core one contraction slice of every operand, and the
    cores' partial sums are accumulated into a shared output slice. That
    accumulation mis-addresses when an operand lays out the dims it shares with
    the output (and its stick) in a different device-dim order than the output:
    ``x.view(B * M, K).mm(w)`` on a 3D device tensor keeps the base layout
    ``[M, K-stick, B]`` while ``mm`` writes a fresh ``[N-stick, B*M]`` buffer,
    and a forced K-split then returns ~26% wrong elements. The identical split
    is correct whenever the orders agree (contiguous 2D input, 3D ``bmm``, a
    broadcast 2D weight), so only the mismatch is restricted. This is a
    lowering limit, independent of co-optimization, which merely picks the
    K-split more often than the work-division cost model.
    """
    if not isinstance(ctx.op.data, Reduction):
        return ConstraintResult()
    if ctx.op.data.reduction_type not in (BATCH_MATMUL_OP, BATCH_MATMUL_FP8_OP):
        return ConstraintResult()
    all_tds = ctx.input_tds + [ctx.output_td]
    if has_qfp8wt_tensor(all_tds):
        # 2D-stick layouts: qfp8wt_matmul_k_split_domains already pins K.
        return ConstraintResult()

    def non_stick_syms(td: "TensorDep") -> set[Symbol]:
        stick_syms = td.device_coords[-1].free_symbols
        return {
            sym
            for coord in td.device_coords[:-1]
            if not coord.free_symbols & stick_syms
            for sym in coord.free_symbols
            if sym in ctx.it_space
        }

    output_syms = non_stick_syms(ctx.output_td)
    for td in ctx.input_tds:
        shared = non_stick_syms(td) & output_syms
        if _layout_dim_order(td, shared) != _layout_dim_order(ctx.output_td, shared):
            return ConstraintResult(
                allowed_splits={v: frozenset({1}) for v in ctx.reduction_vars}
            )
    return ConstraintResult()

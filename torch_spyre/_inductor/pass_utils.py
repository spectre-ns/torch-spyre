...
def _per_core_view_on_buf(
    ...
    # Any non-tiled layout (e.g. MutationLayoutSHOULDREMOVE for in-place
    # writes) has no device geometry to slice. Return an empty view, but
    # carry the op-level has_partial_reduction flag so a partial-reduction
    # write-dep still forces a mismatch in the caller and keeps the buffer
    # off LX.
    if not isinstance(buf_layout, FixedTiledLayout):
        result = (
            PerCoreView(work_slice_dims=(), core_to_slot=()),
    ...
    # Map each device stride to a single device dim. If multiple device dims
    # share the same stride, keep the first one but let a later non-degenerate
    # dim (device_size != 1) override it — this avoids a degenerate axis
    # clobbering the real dim that should host the split ("last in dict" /
    # duplicate-strides fix). Skip non-positive stride_map entries (sentinels).

...
@unittest.skipUnless(_HAS_ORTOOLS, "cpsat placement unit tests need ortools")
class TestCpSatNoOverlap2DEncoding(TestCase):
    """The LX boxes of spilled buffers must keep ``end == start + size``.

    OR-Tools' no_overlap_2d adds "at most one of i-below-j, j-below-i" clauses
    for optional boxes of fixed size (diffn.cc, AddNonOverlappingRectangles)
    without guarding them by presence. A box whose address top is a free
    variable while its buffer is spilled can satisfy both precedences, and
    CP-SAT then proves wrong optima. In the parallel search, LNS fixes spilled
    buffers' variables to earlier solutions' values, which is exactly the
    parking these tests reproduce with one worker.
    """

    @staticmethod
    def _cases():
        r, d = sympy.symbols("r d")
        # Same per-core footprint under both divisions: presolve fixes the size,
        # so OR-Tools treats the box as fixed-size.
        equal = [
            CoreDivision(),
            CoreDivision(splits={r: 2}, reduction_syms=frozenset({r})),
        ]
        different = [CoreDivision(), CoreDivision(splits={d: 2})]
        return {
            "plain": [LifetimeBoundBuffer(f"b{i}", 8, [i, i + 2]) for i in range(3)],
            "equal footprints": [
                CoreDivisionBuffer(f"c{i}", 8, [0, 2], core_divisions=list(equal))
                for i in range(3)
            ],
            "different footprints": [
                CoreDivisionBuffer(f"e{i}", 8, [0, 2], core_divisions=list(different))
                for i in range(3)
            ],
        }
    ...
    @staticmethod
    def _build(buffers):
        """The production placement model (no objective) and its wrappers."""
        solver = CpSatLayoutSolver(buffers, 100, 1)
        model = cp_model.CpModel()
        working = {b.name: solver._wrap(model, b) for b in buffers}
        solver._add_inplace_relaxation(model, working)
    ...
    def _y_boxes(model):
        return [
            c
            for c in model.proto.constraints
            if c.has_interval() and c.name.startswith("y_")
        ]

    @staticmethod
    def _expr(model, proto):
        return proto.offset + sum(
            coeff * model.get_int_var_from_proto_index(var)
            for var, coeff in zip(proto.vars, proto.coeffs)
        )
    ...
    def _sound_solver():
        ...
        # Without the precedence Booleans the unguarded clauses are never added.
        solver.parameters.no_overlap_2d_boolean_relations_limit = 0
        return solver

    def test_parked_spilled_buffers_keep_the_optimum(self):
        for name, buffers in self._cases().items():
            with self.subTest(name):
                model, working = self._build(buffers)
                boxes = self._y_boxes(model)
                box_vars = sorted(
                    {
                        v
                        for c in boxes
                        for v in (*c.interval.start.vars, *c.interval.end.vars)
                    }
                )
                # A feasible "everything spilled at address 0" assignment, found
                # without the unguarded clauses.
                park = model.clone()
                for w in working.values():
                    park.add(park.get_bool_var_from_proto_index(w.in_buffer.index) == 0)
                    park.add(park.get_int_var_from_proto_index(w.offset.index) == 0)
                park.minimize(
                    sum(park.get_int_var_from_proto_index(v) for v in box_vars)
                )
                solver = self._sound_solver()
                self.assertEqual(solver.solve(park), cp_model.OPTIMAL)
                parked = {
                    v: solver.value(park.get_int_var_from_proto_index(v))
                    for v in box_vars
                }
                # Fix the spilled boxes to it, as an LNS neighbourhood would, and
                # minimise residency: spilling everything (objective 0) stays
                # feasible, so the default search must find it.
                for c in boxes:
                    present = model.get_bool_var_from_proto_index(
                        c.enforcement_literal[0]
                    )
                    for v in (*c.interval.start.vars, *c.interval.end.vars):
                        var = model.get_int_var_from_proto_index(v)
                        model.add(var == parked[v]).only_enforce_if(~present)
                model.minimize(sum(w.in_buffer for w in working.values()))
                solver = cp_model.CpSolver()
                solver.parameters.num_workers = 1
                self.assertEqual(solver.solve(model), cp_model.OPTIMAL)
                self.assertEqual(solver.objective_value, 0)

    def test_spilled_box_end_is_start_plus_size(self):
        for name, buffers in self._cases().items():
            model, _ = self._build(buffers)
            for c in self._y_boxes(model):
                with self.subTest(name, box=c.name):
                    probe = model.clone()
                    present = probe.get_bool_var_from_proto_index(
                        c.enforcement_literal[0]
                    )
                    start = self._expr(probe, c.interval.start)
                    size = self._expr(probe, c.interval.size)
                    end = self._expr(probe, c.interval.end)
                    probe.add(present == 0)
                    probe.add(end != start + size)
                    self.assertEqual(
                        self._sound_solver().solve(probe), cp_model.INFEASIBLE
                    )
